...
Enhanced with database storage, trip management, and menu system.
"""
...
class MapRenderer:
    """Map rendering with overlays for e-paper display"""
    
    def __init__(self, mbtiles_path, width=800, height=480):
        self.width = width
        self.height = height
        self.mbtiles_reader = MBTilesReader(mbtiles_path)
    ...
    def render_map(self, lat, lon, zoom, heading=0, wifi_status=None, gps_status=None, map_points=None):
        ...
        try:
            # Generate base map
            image_data, metadata = self.mbtiles_reader.generate_composite_image(
            ...
            draw.text((10, 40), f"Map Error: {str(e)}", fill=0, font=self.font_medium)
            return image, {}
    ...
    def _add_overlays(self, image, lat, lon, zoom, heading, metadata, wifi_status=None, gps_status=None, map_points=None):
    ...
    def _geo_to_screen(self, lat, lon, center_lat, center_lon, zoom):
        ...
        # This is a simplified conversion - you might want to use the same
        # coordinate system as your MBTiles reader
        center_x, center_y = self.mbtiles_reader.deg2num(center_lat, center_lon, zoom)
        point_x, point_y = self.mbtiles_reader.deg2num(lat, lon, zoom)
    ...
    def _draw_info_panel(self, draw, lat, lon, zoom, metadata):
        ...
        if metadata:
            ...
            draw.text((10, panel_top + 50), tile_text, fill=0, font=self.font_small)
        
        # Timestamp
        timestamp = datetime.now().strftime("%H:%M:%S")
        draw.text((self.width - 100, panel_top + 10), timestamp, fill=0, font=self.font_small)
    
    def close(self):
        """Close MBTiles reader"""
        self.mbtiles_reader.close()

class NavigationSystem:
    """Main navigation system controller with enhanced trip management"""
    
    def __init__(self, mbtiles_path, config_file='navigation_config.json'):
        ...
        if action == 'trip_selected':
            ...
            if selected_trip:
                ...
                if self.wifi_manager.is_connected:
                    threading.Thread(target=self._sync_trip_status, args=(selected_trip['id'], 'IN_ROUTE'), daemon=True).start()
            ...
            if selected_trip:
                ...
                # Sync trip status if WiFi available
                if self.wifi_manager.is_connected:
                    threading.Thread(target=self._sync_trip_status, args=(selected_trip['id'], 'COMPLETED'), daemon=True).start()
    ...
    def _update_display(self):
        ...
        try:
            ...
            self._add_gps_overlay(map_image, gps_status, compass_heading, wifi_status)
    ...
    def _add_gps_overlay(self, image, gps_status, compass_heading, wifi_status):
        """Add GPS information overlay"""
        draw = ImageDraw.Draw(image)
        ...
        if self.active_trip:
            ...
            draw.text((x_pos, y_start + 80), trip_text, fill=0, font=self.map_renderer.font_small)
            draw.text((x_pos, y_start + 100), status_text, fill=0, font=self.map_renderer.font_small)
        ...
        if self.sync_manager.is_enabled():
            ...
            draw.text((x_pos, y_start + 120), sync_text, fill=0, font=self.map_renderer.font_small)
        
        # Queue status
        unsynced_count = len(self.db.get_unsynced_logbook_entries())
        if unsynced_count > 0:
            queue_text = f"Queue: {unsynced_count}"
            draw.text((x_pos, y_start + 140), queue_text, fill=0, font=self.map_renderer.font_small)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='GPS Navigation System with Trip Management')
    ...
    nav_system = NavigationSystem(args.mbtiles_file, args.config)
    nav_system.run()
